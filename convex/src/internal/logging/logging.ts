...
export const sendToPostHog = action({
	...
	handler: async (ctx, args) => {
		...
		try {
			...
			const posthogEndpoint = process.env.POSTHOG_ENDPOINT || "https://us.i.posthog.com/i/v0/e/";
			...
			console.log(`PostHog error sent successfully with status ${response.status}.`);
