...
export class ErrorCodes {
  ...
  static readonly REQUEST_HEADER_FIELDS_TOO_LARGE = {
    ...
    message:
      "The server is unwilling to process the request due to its header fields being too large.",
    status: 431,
  };
...
export async function logAndReturnError(
  ...
  if (ctx.scheduler) {
    ...
    await ctx.scheduler.runAfter(
      0,
      "src/internal/logging/logging:sendToPostHog",
      {
        message,
    ...
    await ctx.env.scheduler.runAfter(
      0,
      "src/internal/logging/logging:sendToPostHog",
      {
        message,
