...
export const syncAccountAccessFromMonday = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		const currentUserIds = new Set(currentAccess.map((a) => a.userId.toString()));
		...
		const validContactUsers = contactUsers.filter((u): u is NonNullable<typeof u> => u !== null);
		const newUserIds = new Set(validContactUsers.map((u) => u.userId.toString()));
