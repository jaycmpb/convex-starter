...
export const getRootFolders = query({
	...
	handler: async (ctx, args) => {
		return await ctx.db
			.query("folders")
			.withIndex("by_accountId_depth", (q) => q.eq("accountId", args.accountId).eq("depth", 0))
...
export const getSubfolders = query({
	...
	handler: async (ctx, args) => {
		return await ctx.db
			.query("folders")
			.withIndex("by_parentFolderId", (q) => q.eq("parentFolderId", args.parentFolderId))
			.filter((q) => q.eq(q.field("deletedAt"), undefined))
			.collect();
	},
});
	...
	handler: async (ctx, args) => {
		...
		const [subfolders, documents] = await Promise.all([
			args.folderId
				? ctx.db
						.query("folders")
						.withIndex("by_parentFolderId", (q) => q.eq("parentFolderId", args.folderId))
						.filter((q) => q.eq(q.field("deletedAt"), undefined))
						.take(limit)
				: ctx.db
						.query("folders")
			...
			args.folderId
				...
				: // Root-level documents (no folder) for the account.
					ctx.db
						...
						.filter((q) => q.and(q.eq(q.field("deletedAt"), undefined), q.eq(q.field("folderId"), undefined)))
						.take(limit),
		]);
...
export const getBreadcrumbs = query({
	...
	handler: async (ctx, args) => {
		...
		const breadcrumbs = ancestors.filter((f): f is Doc<"folders"> => f !== null && !f.deletedAt);
...
export const folderNameExists = query({
	...
	handler: async (ctx, args) => {
		const folders = await ctx.db
			.query("folders")
			.withIndex("by_accountId_parentFolderId", (q) => q.eq("accountId", args.accountId).eq("parentFolderId", args.parentFolderId))
			.filter((q) => q.eq(q.field("deletedAt"), undefined))
			.collect();

		return folders.some((f) => f.name.toLowerCase() === args.name.toLowerCase() && f._id !== args.excludeFolderId);
	},
});
