...
export const moveFolder = mutation({
	...
	handler: async (ctx, args) => {
		...
		const descendants = await ctx.db
			.query("folders")
			.filter((q) => q.and(q.eq(q.field("deletedAt"), undefined), q.neq(q.field("_id"), args.id)))
			.collect();

		// Filter descendants manually (path prefix match).
		const descendantFolders = descendants.filter((f) => f.path.startsWith(oldPath + "/"));
...
export const deleteFolder = mutation({
	...
	handler: async (ctx, args) => {
		...
		const descendantFolders = allFolders.filter((f) => f.path.startsWith(folder.path + "/"));
...
export const restoreFolder = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (args.restoreContents) {
			...
			const descendantFolders = allFolders.filter((f) => f.path.startsWith(folder.path + "/") && f.deletedAt);
