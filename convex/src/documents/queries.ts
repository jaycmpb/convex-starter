...
export const documentNameExists = query({
	...
	handler: async (ctx, args) => {
		...
		return documents.some((d) => d.name.toLowerCase() === args.name.toLowerCase() && d._id !== args.excludeDocumentId);
	},
});
...
export const searchDocuments = query({
	...
	handler: async (ctx, args) => {
		...
		return documents.filter((d) => d.name.toLowerCase().includes(searchLower)).slice(0, limit);
	},
});
